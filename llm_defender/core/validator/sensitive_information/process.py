    ...
    # Calculate score for valid response
        ...
        if response.output["subnet_version"]:
            if response.output["subnet_version"] > validator.subnet_version:
                LLMDefenderBase.utils.subnet_logger(
                    severity="WARNING",message=f'Received a response from a miner with higher subnet version ({response.output["subnet_version"]}) than yours ({validator.subnet_version}). Please update the validator.', 
                    log_level=log_level
                )
...
def calculate_analyzer_score(
    ...
    if distance_score is None:
        LLMDefenderBase.utils.subnet_logger(
            severity="DEBUG",
            message=f"Received an invalid response: {response} from hotkey: {hotkey}", 
            log_level=log_level
        )
        distance_score = 0.0
    ...
    LLMDefenderBase.utils.subnet_logger(
        severity="DEBUG",
        message=f"Calculated score: {score_logger}",
        log_level=log_level
    )
...
def apply_penalty(validator, response, hotkey, target, log_level) -> tuple:
    ...
    formatting += LLMDefenderCore.sensitive_information_penalty.check_formatting_penalty(
        uid, response, log_level
    )
...
def get_response_penalties(validator, response, hotkey, target, log_level):
    ...
    if formatting_penalty >= 20:
        speed_penalty_multiplier = 0.0
    elif formatting_penalty > 0.0:
        speed_penalty_multiplier = 1 - (formatting_penalty / 20.0)

    return distance_penalty_multiplier, speed_penalty_multiplier